...
<!-- 
Document generated by Claude Code in Research mode with the following prompt:

  Look for up-to-date references about modern harness engineering practices. Ensure you do not miss articles from well-respected, experienced software developers or software development teams heavily using AI in their projects, like AI companies. Focus on lightweight approaches using mostly plain folders and markdown files committed in the development repository, instead of heavyweight solutions requiring external databases or non-portable and complicated tools (single binary and small tools easily instalable within the development environment are accepted). Write a summary of your findings and propose a simple, practical, and lightweight solution for harness engineering that can be easily adopted by development teams, both in green-field and brown-field projects, including folder structure and file naming conventions. The harness should follow best practices for maintainability, scalability, and ease of use, for any coding agent following community standards. Specific optimizations for Claude Code or OpenCode can be added on top, if they provide significant benefits without adding complexity. If you have questions about the proposal, or there are several viable alternatives, ask for clarification before proceeding.
-->
